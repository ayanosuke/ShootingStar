...
// 2021/12/18 ファンクション変更、MP3データ変更
//
// ArduinoUNOモードで書き込みます
//
// 0001.mp3 登場
// 0002.mp3 プロローグ
// 0003.mp3 夜にかける
...
SoftwareSerial DccSIO(11, 12); // RX, TX
...
void setup() {
  ...
  myDFPlayer.volume(volume*4);  //Set volume value. From 0 to 30
...
//    key = Serial.read();            // 1文字だけ読み込む
  ...
  if ( DccSIO.available() ) {       // 受信データがあるか？
    key = DccSIO.read();            // 1文字だけ読み込む


    switch(key){
      case '1': state = 300;  // Func1 ホーン
                break;
      case '2': state = 200;  // Func2 停止
                break;
      case '3': state = 100;  // Func3 高速走行　走行時
                break;
      case '4': state = 0;    // Func4　オープニング（虹色）
                break;
      case '5': state = 2;    // Func5　プロローグ
                break;      
      case '6': state = 16;    // Func6　夜に駆ける/YOASOBI
                break;    
      case '7': state = 34;    // Func7 アイノカタチfeat.HIDE(GReeeeN)/MISIA
                break;
      case '8': state = 44;    // Func8 天体観測/BUMP OF CHICKEN
                break;                                
      case '9':               // Func9 音量-
                volume --;
                if(volume <= 0){
...
default:
 ...
 // myDFPlayer.loop(1); // オープニング
 // myDFPlayer.play(2); // 2:00​- プロローグ  
 // myDFPlayer.play(3); // 3:45​- ♪夜に駆ける/YOASOBI
 // myDFPlayer.play(4); // 5:42​- ♪アイノカタチfeat.HIDE(GReeeeN)/MISIA
 // myDFPlayer.play(5); // 10:00​- ♪天体観測/BUMP OF CHICKEN
 // myDFPlayer.play(6); // エンディング
